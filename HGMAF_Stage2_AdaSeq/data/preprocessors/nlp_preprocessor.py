...
class NLPPreprocessor(Preprocessor):
    ...
    def __init__(
        ...
        parse_aux_data: bool = True,
        aux_data_field: str = 'aux_data',
        **kwargs
    ) -> None:
        ...
        self.passthrough_fields = tuple(passthrough_fields or [])
        self.parse_aux_data = parse_aux_data
        self.aux_data_field = aux_data_field
    ...
    def __call__(self, data: Union[str, List, Dict]) -> Dict[str, Any]:
        ...
        meta = dict(data)
        aux_payload = self._parse_aux_data(meta)
        if aux_payload:
            for key, value in aux_payload.items():
                meta.setdefault(key, value)
            meta[self.aux_data_field] = aux_payload
        ...
        if self.passthrough_fields:
            passthrough = self._collect_passthrough_fields(data, aux_payload)
            output_dict.update(passthrough)
    ...
    def _parse_aux_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Parse optional serialized sidecar metadata.

        The dataset builder stores arbitrary multimodal metadata as a JSON string
        in `aux_data`. We parse it here so future HGMAF inputs can be passed
        through without changing the dataset schema again.
        """
        if not self.parse_aux_data:
            return {}

        aux_data = data.get(self.aux_data_field)
        if aux_data in (None, ''):
            return {}
        ...
        if isinstance(aux_data, str):
            try:
                parsed = json.loads(aux_data)
            except json.JSONDecodeError:
                logger.warning('Failed to parse `%s` as JSON sidecar metadata.', self.aux_data_field)
                return {}
            return parsed if isinstance(parsed, dict) else {}

        return {}

    def _collect_passthrough_fields(
        self, data: Dict[str, Any], aux_payload: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """Expose selected multimodal fields as top-level model inputs."""
        aux_payload = aux_payload or {}
        passthrough = {}
        for field in self.passthrough_fields:
            if field in data:
                passthrough[field] = data[field]
            elif field in aux_payload:
                passthrough[field] = aux_payload[field]
        return passthrough
